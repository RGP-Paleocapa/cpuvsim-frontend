...
const Page5 = () => {
  ...
  const { data, loading, error } = useFetchData('https://cpuvsim.herokuapp.com/api/data/page5');
  ...
  const bodyRows = data ? data.bodyRows : [];
  const dataFlowRows = data ? data.dataFlowRows : [];
  const controlFlowRows = data ? data.controlFlowRows : [];
  const arithmeticLogicRows = data ? data.arithmeticLogicRows : [];    
    ...
    <EBookPage currentPage={5}>
        <Section>
          ...
          <div className="mt-6 xl:mx-40">
            <Table headerTitles={["Binary code", "Mnemonic code", "Description"]} bodyRows={bodyRows as string[][]} />
          </div>
        </Section>
        ...
        <Section>
          ...
          <div className="mt-6 xl:mx-40">
            <Table headerTitles={["Data Flow"]} bodyRows={dataFlowRows as string[][]} />
            <Table headerTitles={["Control Flow"]} bodyRows={controlFlowRows as string [][]} />
            <Table headerTitles={["Arithmetic-logic"]} bodyRows={arithmeticLogicRows as string [][]} />
          </div>
        </Section>
    </EBookPage>
  );
